...
// kept here for wrapping of Terminal.ANSI + Strings.Alingable
public func makeQuickSummary(
    ...
    for value in values {
        let string = 
            leftAlignText(
                value.name.ansi(.bold), 
                width: 25
            ) 
            + 
            rightAlignText(
                "\(value.amount)".ansi(.bold),
                width: 25
            )
    ...
    let box = createBox(for: stringToBox, callback: stripANSI)
